...
public class StatController {
        ...
        while (inProcess.before(today)) {
            ...
            for (Empleado emp : empleados) {
                ...
                if (emp.getFechaIngreso().before(inProcess.getTime())
                    ...
                    sumaSalarios += emp.getSalario() * emp.getDias_jornada();
                    ...
                    for (Deduccion ded : deducciones) {
                        ...
                        if (ded.getFecha().before(inProcess.getTime()) && ( //Y la fecha de finalizacion es nula o bien posterior
                    ...
                    for (Percepcion per : percepciones) {
                        //Si la fecha de inicio es anterior a la que es procesada
                        if (per.getFecha().before(inProcess.getTime()) && ( //Y la fecha de finalizacion es nula o bien posterior
    ...
    public ChartPanel getEmployeesStatGraphic(ListaNomina sel) {
        List<Empleado> allEmpleados = new EmpleadoController().getAllActualEmpleados();
        List<Empleado> empleados = new ArrayList<>();

        allEmpleados.stream()
                .filter((emp) -> (emp.getNomina() == sel.getId()))
                .forEach((emp) -> {
                    empleados.add(emp);
                });
        ...
        empleados.stream().forEach((emp) -> {
            data.addValue(emp.getSalario() * emp.getDias_jornada(), "Empleados", emp.toString());
        });
    ...
    public ChartPanel getEmployeeStatGraphic(List<Empleado> empleados) {
        DefaultCategoryDataset data = new DefaultCategoryDataset();
        empleados.stream().forEach((Empleado emp) -> {
            data.addValue(emp.getSalario() * emp.getDias_jornada(), "Salario", emp.toString());
    ...
    public ChartPanel getNominaStatGraphic() {
        ...
        empleados.stream().forEach(emp -> {
            ...
            dinero[nom] += emp.getSalario() * emp.getDias_jornada();
        });
    ...
    public ChartPanel getDeptoStatGraphic() {
        ...
        empleados.stream().forEach(emp -> {
            ...
            dinero[dep] += emp.getSalario() * emp.getDias_jornada();
        });
